...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'julie')
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*Codded by AXN4*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*Codded by AXN4*", quoted: message.data })
